...
- **Questions d'armes** (ajout du 17/09/2026) : collection **commune**, `src/content/commun/questions/`, des pages question-réponse indexables dont l'objectif est le référencement (les requêtes « épée longue », « messer arme », « rapière » font des centaines d'impressions en Search Console). Champs : la question (titre), statut brouillon/publié, date, armes concernées (multi-sélection dans les disciplines, comme les traités), la réponse en **corps libre** (5ᵉ champ à widgets — même jeu exact que les quatre autres, règle de `src/cms/widgets.ts`), un bloc facultatif « Appuyé sur » (libellé, URL, ligne de crédit rendue **verbatim**, régime des crédits patrimoniaux), et une case « dérivé du wiki FFAMHE » qui affiche la mention de licence CC BY-NC-SA et engage le partage à l'identique. Pages `/questions/` (sommaire groupé par arme, ordre du catalogue) et `/questions/<slug>/`. Balisage **FAQPage** (jamais QAPage, réservé par Google aux réponses soumises par les visiteurs) + BreadcrumbList — arbitrage documenté dans `src/lib/jsonld.ts`. Maillage : encadré « Questions sur cette arme » sur les fiches `/armes/`, renvois automatiques vers la fiche arme et les traités liés depuis chaque question, lien depuis `/sources/`, entrée « Questions d'armes » dans la colonne « Le club » du pied de page — **pas** d'entrée dans la nav principale ni la TabBar (l'entrée FAQ existante est la FAQ pratique du club, à ne pas cannibaliser). Rédaction : réponses **originales** (jamais de copier-coller du wiki, contenu dupliqué que Google ignore), 150-300 mots, chaque affirmation historique vérifiable dans une source citée. Mode d'emploi rédacteur : GUIDE-ADMIN.md §12.
...
### La photo de « Le club » — deux compositions, choisies au build

- La section « Le club » (`Club.astro`) sait recevoir une **photo panoramique** — celle du club au complet, 1920 × 690 — sans la mutiler : le split maquetté (`object-fit: cover`, cadre 688/430) en écarterait 43 % en desktop et la moitié en mobile, où l'assise du titre incrusté ne tient de toute façon pas sur un cadre si bas.
- Le choix est fait **au build, jamais dans l'admin** : `resoudrePhoto` donne les dimensions naturelles du fichier, et au-delà d'un ratio de **2:1 strict** (`SEUIL_RATIO_PANORAMA`, justifié sur rendus d'essai dans le composant : le 2:1 pile tient encore dans le split, 16:9 y est net) la section bascule en composition « panorama » — titre et corps en tête, puis la photo **entière**, à son ratio naturel, sur la largeur utile de la section, crédit au ras ; le titre ne s'incruste jamais dans la photo dans cette variante, mobile compris. Chiffres et piliers ne bougent dans aucune des deux compositions.
- Même logique que l'art direction du hero (ci-dessus), mécanisme différent : le hero choisit **au chargement** entre deux fichiers selon l'écran (`<picture>`), « Le club » choisit **au build** une composition selon le format du fichier unique. Remplacer la photo dans l'admin suffit — une photo classique reprend le split à l'identique, un panorama s'affiche entier, rien à retoucher côté code.
